...
const royalPics = [
  [
   'https://i.postimg.cc/VsF75DvJ/Screenshot-20230516-225647-Whats-App.png'
  ], [
    "https://i.postimg.cc/SRLbzqwH/IMG-20230516-WA0007.jpg"
  ]
]
//fetch pictures 
fetch('pictures.json')
...
<div class='floatingTabs'>
    ...
    partyAnimalsPics.forEach((eachPartyAnimal, currentPartyAnimalIndex, arr) => {
      eachPartyAnimal.addEventListener('click', (e) => {
        ...
        //enlarging partyAnimalPic
          ...
          preview.classList.add('previewShow')
          let image = pictures[currentPartyAnimalIndex][1];
    ...
    king.style.backgroundImage = `url(${royalPics[0][0]})`;
    queen.style.backgroundImage = `url(${royalPics[1][0]})`
    ...
    container.addEventListener('scroll', (s) => {

      //parallax venue
      // let v=window.scrollX;
      //venue.style.marginLeft=v/2+'px'
...
imageBtns.forEach((e) => {
  ...
  slideShowImages.classList.remove('slide');
    ...
    if (tar == 'previousImage') {
      if (slideShowImages.classList.contains('slide')) {
    slideShowImage.classList.remove('slide');
